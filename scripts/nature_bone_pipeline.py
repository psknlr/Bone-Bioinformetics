...
def plot_all(figdir, mods, hist, prox, cell, genetics):
    ...
    plt.figure(figsize=(8,5)); c2=cell.copy(); c2["plot_cell"]=c2.cell_state.replace({"BMSC/成骨祖细胞":"BMSC","成骨细胞":"Osteoblast","骨细胞":"Osteocyte","破骨细胞":"Osteoclast","破骨前体/巨噬细胞":"OC-precursor/Mac"}); sns.barplot(data=c2,x="plot_cell",y="marker_overlap_fraction",color="#1b9e77"); plt.xticks(rotation=30,ha="right"); plt.ylabel("marker overlap fraction (not UCell)"); plt.tight_layout(); plt.savefig(figdir/"Fig4_curated_marker_overlap.png",dpi=300); plt.close()
