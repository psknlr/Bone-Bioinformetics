...
[![Open in Colab](https://colab.research.google.com/assets/colab-badge.svg)](https://colab.research.google.com/github/OWNER/Bone-Bioinformetics/blob/main/notebooks/bone_bioinformatics_colab.ipynb)
...
A reproducible Colab notebook is provided at `notebooks/bone_bioinformatics_colab.ipynb`. If the badge URL still contains `OWNER`, replace it with the GitHub organisation/user that hosts this repository after pushing. The notebook installs dependencies, runs the real-data pipeline, displays generated figures, and previews the key CSV outputs. See `docs/REAL_DATA_AUDIT.md` for a source-by-source audit of which analyses use real data and which high-order modules are intentionally marked as pending full summary-statistics/QTL/perturbation inputs rather than simulated.
