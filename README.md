...
1. **Genetics-anchored multi-omics causal pharmacology** — pulls Open Targets `genetic_association` datatype scores per target (the real human-genetics anchor, separated from literature/animal evidence), maps symbols to Ensembl via MyGene, and runs a **real cis-eQTL presence check** against a curated set of bone/immune-relevant eQTL Catalogue datasets (per-dataset REST path). It re-grades candidates A/B/C where **A requires real human-genetics support** plus pharmacology. An honest result surfaces here: the herb single-protein targets carry **no Open Targets `genetic_association` for osteoporosis** (the genetically-supported bone genes — COL1A1, LRP5, SPP1, TNFRSF11A — are a different set), so none reach A. Fine-mapping/coloc/SMR/MR remain pending on full sumstats/QTL. Outputs: `m1_genetics_anchored_causal_targets.csv`, `m1_opentargets_genetic_datatypes.csv`, `m1_causal_pending_steps.csv`.
